...
# PREVIEW DATA
glimpse(members)
skim(members) # summary statistics grouped by variable type

# RESEARCH QUESTION:
# What injuries are associated with climbing Everest?
        #SUB-COMPONENTS:
        # How many people have been injured while climbing Everest?
        # What type of injuries occurred?
        # Where are injuries occurring (elevation)?
...
# SUB-COMPONENT 1: How many people have been injured while climbing Everest?

members %>%
        filter(peak_name == "Everest" & injured == "TRUE") %>%
        count(injured)

        # RESULT: 485 injuries while climbing Everest.
...
# Plot 1 - un-binned attempts
        ...
        # Jitter the data points (lose specificity elevation) to show density
        ggplot(members, aes(x = injury_type, y = injury_height_metres)) + 
                geom_quasirandom() 
        ...
        # dot color/size density - inverse colors
        ggplot(members, aes(x = injury_type, y = injury_height_metres)) + 
...
#MOVE FORWARD WITH BINNED DOT DENSITY
...
r_members <- members %>%
        ...
        # SUB-ANALYSIS: Count NAs by injury type
        nas <- members %>%
                filter(injured == "TRUE") %>%
                mutate(ihm_100000s = round(injury_height_metres/100000)*100000) %>%
                # Bin "values" vs "NA"s
                count(injury_type, ihm_100000s) %>%
                group_by(injury_type) %>%
                mutate(freq = n / sum(n))
                         
# Data - Counts of injury by bins
c_data <- r_members %>%
        filter(injured == "TRUE") %>%
        group_by(injury_type, ihm_100s) %>%
        summarise(count = n()) %>%
...
# Plot
p <- ggplot(c_data, aes(x = injury_type, y = ihm_100s, color = count, size = count)) +
...
p + labs(title = "Injury in the Himalayas by elevation", 
    ...
    scale_x_discrete(labels = c("AMS", 
                                    "Avalanche",
                                    "Crevasse",
                                    "Exhaustion",
                                    "Exposure\n/ frostbite", #line breaks in values
                                    "Fall",
                                    "Falling \nrock / ice",
                                    "Icefall\ncollapse",
                                    "Illness\n(non-AMS)",
