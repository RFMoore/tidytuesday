...
# Sub-components: How many "seasons" are there? 
                ...
                # Visualize comparison of results year-by-year (facet_wrap)?
...
friends %>%
        group_by(speaker) %>%
        filter(speaker == "Monica Geller" | # EXPLORE more graceful means
...
# Sub-component 3: Visualize comparison of results year-by-year?
...
f_count <- friends %>%
        ...
        filter(speaker == "Monica Geller" | 
                       ...
                       speaker == "Ross Geller" |
                       speaker == "Rachel Green") %>%
        add_count(speaker, season, name = "utterance_per_season") # New count is         unassigned. By changing "count" to "add_count" create a new variable            within original dataset and assign a name.

p <- ggplot(f_count, aes(x = season, y = utterance_per_season, color = speaker)) + 
        geom_line() +
...
p + ggtitle("I tend to keep talking until somebody stops me", subtitle = "Frequency of dialogue by Friends character per season") + xlab("Season") + ylab("Dialogue") + legend("Character")
